...
**2026-08-04** — **the Codex lane answers streaming requests with an actual stream.** Asked for `stream: true` the OAuth shim returned a single complete body, so LiteLLM emitted one empty chunk and every `cod-*` model looked dead in any streaming client — which is all of them. Non-streaming callers worked, which is exactly why it went unnoticed. The shim now translates the upstream Responses events into `chat.completion.chunk` frames as they arrive, with an opening role chunk, reasoning kept off `content`, tool-call indices and a terminating `[DONE]`. Also: the opencode model picker is now generated from `/v1/models` by `scripts/sync_opencode_models.sh` instead of hand-kept — it had drifted to 43 declared against 74 served, so a quarter of the roster was unselectable while `auto` kept working and hid it.

Earlier entries, condensed:

- **2026-08-03** — `gpt-5.6-luna` added on the GO flat lane; cost cascade puts **Codex ahead of GO** while that ChatGPT plan is free. Multi-machine setup moved to the private companion repo (see Foundations).
- **2026-08-02** — **DeepSeek V4 Flash 0731 becomes the default in every tier** (a *flash* model beating the previous generation's *pro*: Terminal-Bench 82.7 vs 72.1). Its flat twin heads the flat block, so draining free capacity keeps the same model. The two failure-only fallback paths re-sorted to follow the cost cascade.
- **2026-08-01** — free hosts ordered by **reliability, not latency** (Zen free → Mistral → NIM); native reasoners given their own health-probe ceiling so thinking is no longer mistaken for being slow.
- **2026-07-31** — **prompt caching end to end**: per-model hit rates measured from the audit trail, Anthropic's missing `cache_control` fixed (0 → 99.9%), session stickiness, and prompt size measured on the whole payload.
- **2026-07-30** — **aliases renamed so the prefix names the billing lane** (`free-*` · `nim-*` · `mis-*` · `cod-*` · `go-*` · `ant-*` · `zai-*` · `zen-*` · `co-*`), full GO roster wired by quota, Codex/ChatGPT OAuth backend added.
